...
do
	...
	echo; echo "(3) 3x+1 [loop found: 4 2 1]"
	echo; echo "(5) 5x+1"
	echo "    [loops found]"
	echo "    26 13 66 33 166 83 416 208 104 52"
	echo "    136 68 34 17 86 43 216 108 54 27"
	echo "    4 2 1 6 3 16 8"
	echo "    may not loop"
	echo; echo "(7) 7x+1"
	echo "    [loops found]"
...
# Set limits
case $Multiplier in
	3) EndNum1=1; EndNum2=1; EndNum3=1 ;;
	5) EndNum1=1; EndNum2=13; EndNum3=27 ;;
	7) EndNum1=1; EndNum2=1; EndNum3=1 ;;
	9) EndNum1=0; EndNum2=0; EndNum3=0 ;;
esac

# Ask user: run for a certain number of iterations or not
Iterations=""
echo; echo "Enter the number of iterations to run. Helps in finding patterns."
read -p "(press Enter to have end when it hits a defined limit): " Iterations
if [[ $Iterations != "" ]]
then
	EndNum1=0; EndNum2=0; EndNum3=0
fi
...
while [[ $Num != $EndNum1 && $Num != $EndNum2 && $Num != $EndNum3 ]]
do
	# If increment hits $Iterations, go ahead and exit
	if [[ $Iterations != "" ]]
	then
		if [[ $i -gt $Iterations ]]
		then
	...
	## Loop Detection if Interations was set by the user.
	if [[ $Iterations != "" ]]
	then
		## Add current Num to end of NumList Array
		NumList[$((${#NumList[@]}+1))]=$Num

		## Loop through the NumList Array and see if Num has appeared in any other element
		## If so, a loop has occured. Then Add Num to the LoopNums array.
		NumListLength=$((${#NumList[@]}-1))
		for ((j=0; j<$NumListLength; j++))
		do
			if [[ $Num -eq ${NumList[$j]} ]]
			then
				LoopNums[$((${#LoopNums[@]}+1))]=$Num
			fi
		done
	fi
...
# If there is a value in the LoopNums array element 1, we know there's a loop
if [[ ${LoopNums[1]} ]]
then
	# Loop through all of the numbers stored in the LoopNums array
	NumListLength=$((${#LoopNums[@]}-1))
	for ((j=0; j<$NumListLength; j++))
	do
		# If a number from the LoopNums array does not exist in the LoopList array, add it
		if [[ ! ${LoopList[*]} =~ ${LoopNums[$j]} ]]
		then
			LoopList[$((${#LoopList[@]}+1))]=${LoopNums[$j]}
		fi
	done
	echo; echo "A loop was found! (${LoopList[@]})"
else
	echo; echo "No loop was discovered."
fi
