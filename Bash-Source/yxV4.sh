...
do
	...
	echo; echo "(5) 5x+1"
	echo "    [loops found]"
	echo "    66 33 166 83 416 208 104 52 26 13 (seeds: loop #s, 5, 10, 13)"
	echo "    86 43 216 108 54 27 136 68 34 17  (seeds: loop #s, 27)"
	echo "    6 3 16 8 4 2 1                    (seeds: loop #s, 2^x, 12)"
	echo "    may not loop                      (seeds: 7, 9, 11, 14, 18)"
	echo; echo "(7) 7x+1"
	echo "    [loops found]"
	echo "    8 4 2 1                           (seeds: loop #s, 2^x, 5, 9, 10)"
...
do
	...
	then
		...
		# If Seed is odd, just use what came from $RANDOM
		# If Seed is even, use od to dump /dev/urandom for a larger random number.
		if [[ $Seed%2 -eq 0 ]]
		then
			Seed=$(od -An -N4 -i < /dev/urandom | tr -d ' ')

		        # If Seed is negative, multiply it by -1 to get the absulute value
			# don't just throw it (removes an unneeded loop).
			if [[ $Seed -le 0 ]]
			then
				Seed=$((Seed*-1))
			fi
...
# Ask user: How iterations, default is 10000
Iterations="", count=10000
echo; echo "Enter the number of iterations to run. Script will automatically stop when:"
echo " > A loop is found."
echo " > The result is negative."
echo " > The iteration count is reached."
echo; read -p "(press Enter for the default of $count): " Iterations
if [[ $Iterations == "" ]]
then
	Iterations=$count
fi
...
# Just an interesting thing. Checking to see if 3x+1 or 7x+1 was chosen
# and if the Num being processed is a power of 2
if [[ $Multiplier -eq 3 || $Multiplier -eq 7 ]]
then
	echo
	if [[ $(($Seed & $Seed-1)) == 0 ]]
	then
	        echo "**************************** Intersting factoid time ****************************"
		echo "*            The seed is a power of 2 and the multiplier is $Multiplier.                  *" 
		echo "*               As a result, the only odd number will be 1.                     *"
		echo "*                                                                               *"
		echo "*       This effect happens with 3x+1, 7x+1, 15x+1, 31x+1, etc. - any time      *"
		echo "*      the numbers in the formula, when added together, equal a power of 2.     *"
		echo "*********************************************************************************" 
